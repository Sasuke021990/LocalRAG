...
## Device-testing bug run (2026-07-25/26) — **7 bugs found and fixed**

Found by the user testing on a physical device immediately after Phase 2 + dark mode landed. All on [PR #29](https://github.com/Sasuke021990/LocalRAG/pull/29) (still open at time of writing), 7 commits on top of `450183f`. Every fix has regression tests; where a test asserted on pre-existing behavior it was **verified to fail against the pre-fix code and pass after**, so none of them pass vacuously.

**Two of these (#5, #6) are the same root cause as P1 #8** — React Native's global `fetch` never exposes a readable `body.getReader()`. That was fixed in `api/query.ts` during Phase 2 but the identical bug was sitting in a second place. **Worth checking any future SSE consumer for this**: the failure mode is silent (the code falls into a "no stream support" fallback and reports a fake success), so it looks like working code.

- [x] **1. Launch crash — `ErrorBoundary` crashed itself.** `App.tsx` mounted `SafeAreaProvider` *inside* `ErrorBoundary`, but the boundary's fallback UI renders a `SafeAreaView`, which throws without a provider above it. So any caught error crashed the error screen instead of displaying it. Reordered so `SafeAreaProvider` wraps `ErrorBoundary`. Also added `error.message` to the fallback in dev builds — the original diagnosis needed a Metro log dive that the screen itself should have answered.
- [x] **2. Answers pulled from the wrong pool (conversation history).** `main.py::_history_from` threaded every prior turn of a conversation into the LLM prompt regardless of which pool those turns belonged to. Mobile's `choosePool()` doesn't reset `activeConversationId`, so switching pools mid-chat kept the same conversation — retrieval and cited sources were correctly re-scoped, but the model still saw history discussing the *old* pool and a vague follow-up ("analyse the documents") leaned on it. Now compares the request's pool against the conversation's last-used pool and drops history entirely on a mismatch. 5 tests in `test_query_pool_history.py`.
- [x] **3. Pool filter silently ignored — variable shadowing.** In `hybrid_search.py::search()`, the BM25 auto-load loop did `pool = doc.get('pool')`, **shadowing the method's own `pool` parameter**. Retrieval was then re-scoped to whichever document Redis iterated last instead of the pool the user selected. Only triggered on a cold BM25 cache (first query after a restart, or after `invalidate_bm25` on any upload/delete/move), which is why it presented as intermittent. Renamed to `doc_pool`. 2 tests in `test_hybrid_search.py`. **Distinct from #2** — same user-visible symptom, unrelated cause; both were live simultaneously.
- [x] **4. "Summarise the pool" only covered one of two documents.** Architectural, not a typo: chat retrieval is a flat top-K similarity search (hybrid + rerank) with **no per-document diversity safeguard** anywhere in `hybrid_search.py` or `reranker.py`. For a vague whole-pool question, one document's chunks can fill the entire top-K window and squeeze another document out completely — the model never sees a sentence of it. Added `grounding.is_pool_summary_request()` to detect a whole-pool overview ask (a specific-topic ask like "summarize the refund policy document" still goes through normal retrieval) and route it to a path that uses each document's already-stored summary (§1d) instead of chunks, so every document in scope is represented. 25 tests. **Note**: the underlying no-diversity property still applies to any other whole-pool-ish query that doesn't match the trigger — MMR or a per-document cap would be the general fix, not done.
- [x] **5. Knowledge graph invisible outside the uploading pool → now one unified graph.** Graph storage is keyed per pool (`graph_nodes:<user>:<pool>`), so each pool had an isolated graph and a document uploaded to pool A could never appear while viewing pool B. Per the user's decision, mobile now shows **one merged graph across all pools** with no pool selector: new `graph_store.get_user_graph()` + `GET /graph`. Same concept in two pools collapses to one node (ids are label slugs); `source_count` is a union so a document in both pools isn't double-counted. Also **removed the "Highlight concept" dropdown from the UI only** — tapping a node in the WebView still focuses it and dims unrelated ones (that lives in `graphHtml.ts`, untouched); only the external picker is gone. 7 tests. **`/pools/{pool}/graph` deliberately left intact — the web client still shows the per-pool view** (see open item below).
- [x] **6. Large upload looked hung; progress never actually streamed.** `watchUploadProgress` used RN's global `fetch`, so it always hit the no-reader branch and fired an instant fake "complete" — the server's progress stream was **never consumed at all**, and the bar sat frozen through the entire chunk/embed pass. Switched to `expo/fetch`. Separately, the byte transfer itself reported nothing (fetch can't report upload progress), so an 11 MB send was a multi-second dead stretch at 0% — switched to `XMLHttpRequest` for `upload.onprogress`, with 401 routed through a new `notifyUnauthorized()` export so bypassing `request()` doesn't also bypass the app-wide auto-logout. Both phases now drive one bar (transfer 0-50%, processing 50-100%) so it only moves forwards. Also replaced `KnowledgeScreen`'s empty upload `onError` handler. 8 tests.
- [x] **7. Progress bar vanished mid-upload, then froze at 85%.** Three separate gaps: (a) the SSE window was ~2 minutes and a large document's CPU embed pass outlived it, after which the generator just *ended* with no terminal event — now ~15 min, and every exit path emits a terminal event rather than silence, with the client reconnecting if a stream drops; (b) embedding was one monolithic `model.encode()` over every chunk, silent from 50-70% — now batched, reporting "Embedding chunks N/M", which also refreshes the progress key's TTL so a long embed can't expire it; (c) 85% on the client is server-side 70%, i.e. the moment embedding ends — `_store_in_redis` then looped `index_chunk` **one Redis round trip per chunk**, thousands of them, reporting nothing. Now pipelined in batches of 500 (a real speedup, not just cosmetic) reporting 70→85%, with the JSON backup announced separately. 3 tests.

**Embedding performance — measured, not assumed** (asked during this run). Benchmarked in the actual container, 400 chunks of ~540 chars:

| Setup | Throughput |
|---|---|
| `all-MiniLM-L6-v2`, batch 32 (current default) | **~170 chunks/s** |
| `all-MiniLM-L6-v2`, batch 128 | ~130 chunks/s — *slower*, don't reach for this knob |
| `paraphrase-MiniLM-L3-v2` | ~255 chunks/s (~1.5×, lower recall) |

- [x] `utils/device.py::get_best_device()` was **hardcoded** to `torch.device("cpu")` with no override. Now honours `TORCH_DEVICE` (`cpu`/`auto`/`cuda`/`mps`), defaulting to `cpu` and degrading to CPU with a warning when the requested device is unavailable. `TORCH_DEVICE` + `EMBEDDING_MODEL_NAME` plumbed through `docker-compose.yml` and documented in `.env.example` with the numbers above.
- [ ] **GPU not enabled — recommended *against* pursuing for now.** Verified in-container: torch is `2.13.0+cpu` (no HIP/ROCm) **and** there's no GPU passthrough (`/dev/kfd`, `/dev/dri` absent), so `TORCH_DEVICE=auto` finds nothing today. Enabling it needs a ROCm torch build *and* device passthrough *and* host ROCm drivers. The user's card is an AMD R9700 (RDNA4/gfx1201, 32 GB) on **Windows** — the least-supported combination for ROCm, which is Linux-first. Advice given: skip it, since (a) the actual complaint was the progress bugs above, now fixed, (b) going SaaS means embedding will run on a Linux host where GPU is straightforward, so effort on the Windows dev box doesn't transfer. **Also advised against switching to the L3 model** — ~1.5× isn't worth the recall loss when retrieval quality is the product's whole value proposition.
- [ ] **Web still shows the per-pool graph.** `/pools/{pool}/graph` was left intact so the web client's behavior didn't silently change. Decide whether to unify web too.
