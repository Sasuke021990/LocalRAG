...
### 🟠 P1 — Broken or misleading — **ALL 9 FIXED (2026-07-25, "Phase 2")**
...
- [x] **14. No dark mode — FIXED.** `theme/tokens.ts` now exports a `lightColors`/`darkColors` pair (brand accents identical across both so the product still reads as itself; only the neutral surface/text scale inverts) plus a widened `ColorTokens` type. New `theme/ThemeContext.tsx` provides `useAppTheme()` — follows the OS appearance setting live via `useColorScheme`, no restart and no in-app override for v1; a `null` scheme (before the OS value resolves) falls back to light so the UI never flashes dark on startup. `app.json`'s `userInterfaceStyle` changed from `"light"` to `"automatic"`, and `App.tsx` gained an `AppShell` component (below `ThemeProvider`) that drives the React Navigation theme and status-bar style from the active scheme. **All 27 files** that imported the old static `colors` object were migrated: every `StyleSheet.create()` entry had its color keys stripped out (they were baked in at module-eval time and could never react to a theme change) and re-applied as inline style overrides from `useAppTheme()`. `ErrorBoundary` needed its fallback UI split into a separate function component, since a class component can't call hooks. `graphHtml.ts` takes an optional 4th `theme` param so the WebView graph's inlined CSS matches — and `KnowledgeGraphScreen` includes the scheme in the WebView's `key` so it genuinely remounts on a theme switch. 7 new tests (4 in `ThemeContext.test.tsx`, 3 in `graphHtml.test.ts`).
...
- [x] **16. Zero animation anywhere — FIXED (2026-07-26).** Three reusable primitives, all built on the already-installed (and until now entirely unused) `react-native-reanimated`:
  - `FadeIn.tsx` — fade + 8px rise entrance, 220ms ease-out, runs once on mount. Deliberately subtle: it wraps content that appears constantly (chat bubbles), so anything longer or bouncier gets irritating by the tenth repetition. Applied to `ChatBubble`.
  - `Skeleton.tsx` — pulsing placeholder. Opacity pulse rather than the usual sliding shimmer, since shimmer needs a per-block gradient overlay and reads no more clearly at this size.
  - `StreamingCursor.tsx` — the `▍` caret now actually blinks. Previously static, so a *stalled* stream and a live one looked identical.
  - **Also fixed a real bug found while wiring the skeleton**: the Knowledge screen showed "No documents yet. Upload your first above." during the initial fetch — telling a user with a full library that it was empty until the request resolved. Now shows skeleton rows while `docsQ.isLoading`.
- [x] **17. No haptic feedback — FIXED (2026-07-26).** New `utils/haptics.ts` wrapper over `expo-haptics`, centralising the platform guard and failure-swallowing rather than scattering try/catch at every call site: a device with no taptic engine (or with haptics disabled system-wide) *rejects* these calls, and letting that bubble would turn "no vibration" into an unhandled rejection on an otherwise-successful action. Semantic API (`tapLight`/`tapMedium`/`notifySuccess`/`notifyError`/`selectionChanged`) so call sites express intent, not intensity. Wired to: send (light), stop generating + delete document (medium — interrupting/destructive), copy answer (light), upload start (light), answer complete + upload complete (success), answer failed + upload failed (error). 8 tests — one caught a genuine flaw: the platform check was cached at module load, which happened to work in production only because `Platform.OS` never changes at runtime; now evaluated per call.
- [ ] **18. No accessibility labels anywhere** — zero `accessibilityLabel` usage in the entire codebase; unusable with a screen reader.
- [x] **19. Logout has no confirmation — FIXED (2026-07-26).** Gated behind a confirm `Alert`, matching the existing delete-account pattern. Cache clearing still runs on confirm, so a signed-out device never leaves the previous account's documents/chat visible.
- [x] **20. No visible "New chat" entry point on the Chat screen itself — FIXED (2026-07-26).** Pen icon next to History in the Chat header, calling the already-tested `chatStore.newChat()`. Only rendered when `history.length > 0` (pointless clutter on an already-blank chat) and disabled while a message is still streaming.
- [x] **21. Daily AI-question quota is invisible everywhere except the Billing screen — FIXED (2026-07-26).** New shared `AiQuotaBar.tsx`: a full card on Home, and a compact strip pinned under the Chat header so hitting the limit isn't a surprise mid-conversation. Shares the `['subscription']` query key with the Graph/Billing screens, so an existing plan-change invalidation refreshes it everywhere for free. `chatStore` now invalidates that key after **both** `onDone` and `onError` — the backend counts a question at dispatch, before generation, so a *failed* answer still spends quota and the indicator would otherwise over-report what's left. Warning thresholds (tuned 2026-07-26, was 20%): amber within **50%** of the allowance with a **floor of 3** (the floor matters on small plans — 50% of 5 still rounds to 2, so without it a 5/day plan warns two questions too late), rose at zero. **Free plan skips the amber stage entirely** — normal straight to exhausted, no "running low" step — since its allowance is small enough that an early warning read as nagging rather than useful; paid plans keep it. An over-limit count (possible during a dispatch race) clamps to 0 rather than rendering a negative remainder. A failed fetch renders nothing rather than a wrong number — deliberate for a supplementary indicator, and the screens owning that data surface their own errors. Required extracting the `QueryClient` into `api/queryClient.ts` so non-React code (the chat store) can invalidate. 15 tests (2 verified to fail against the pre-fix code for the Free-plan gate specifically).
- [~] **22. No copy / retry / regenerate / stop-generation controls on chat answers — COPY, RETRY, STOP DONE (2026-07-26); regenerate deferred, see below.**
  - **Stop** — the composer's Send button becomes a rose Stop button while an answer streams. Wired through a real `AbortSignal` passed into `expoFetch`, so it isn't merely a UI nicety: aborting closes the connection, which is exactly what `main.py::query_stream` watches (`is_disconnected()` → `agen.aclose()`) to propagate cancellation into the LLM backend's `cancel_event` and actually stop the inference server generating. A stopped answer **keeps whatever text already streamed** (partial, not failed — no red error state), and the abort is deliberately *not* routed to `onError`, which would otherwise paint a failure over the user's own choice. It also must not fall back to `/query`, or a cancelled question would be re-run and charged to the daily quota a second time.
  - **Copy** — `expo-clipboard`, with an inline "Copied" checkmark for 1.5s rather than a toast/alert (an interrupting dialog is heavier than the action itself).
  - **Retry** — offered only on the **last** bubble (retrying an older one would discard everything after it) and only when that turn **failed or was stopped**. Drops the dead turn before resending so the question doesn't appear twice; safe because `main.py` only calls `_persist_turn` once `final_data` exists, so an errored/stopped turn was never written server-side.
  - Actions are hidden while streaming — copying half an answer or retrying mid-flight are both meaningless; Stop covers that window.
  - [ ] **Regenerate a *successful* answer — deliberately not built.** It would need backend support to *replace* the last stored turn: a completed turn is already persisted, so re-running it appends a duplicate user+assistant pair to the conversation instead of superseding it. Needs a "replace last turn" endpoint (or a `regenerate` flag on `/query`) before this can be done without corrupting transcripts.
  - 12 new tests (6 store, 6 API). Verified non-vacuous by neutralising all 6 abort guards in `query.ts` and confirming 3 tests fail before restoring.
- [x] **23. Minor auth-form gaps — FIXED (2026-07-26).** Handled centrally in the shared `ui/Input.tsx` rather than per-screen: it now (a) renders an eye/eye-off reveal toggle on any `secureTextEntry` field, and (b) forwards refs, which is what makes return-key focus chaining possible at all (every auth `TextInput` is wrapped by this component). Wired through Login (email→password→submit), Signup (username→email→password→confirm→submit), Forgot Password, **and** Settings' change-password form, which had the same three-field pattern. Also added `keyboardType="email-address"` where missing plus `autoComplete`/`textContentType` hints so password managers can autofill — `new-password` on signup/change-password so a manager offers to generate one instead of trying to fill an existing credential. Non-password fields keep `secureTextEntry` **undefined** rather than `false`, so nothing changes for the ~6 other screens using this component. 7 tests, one verified to fail against the pre-fix code.
...
2. ✅ **Phase 2 — trust (P1 items 7-15) — DONE 2026-07-25.** Logout hits the backend → real streaming + `res.ok` handling → pull-to-refresh everywhere → auto-refresh after upload → error boundary → offline handling → bigger touch targets → dark mode (added last, after the other 8 shipped in [PR #28](https://github.com/Sasuke021990/LocalRAG/pull/28)). Mobile Jest suite 41/41 passing, `tsc --noEmit` clean, `expo export` bundles successfully.
3. **Phase 3 — delight (P2 items 16-23) — not started.** Animations → haptics → chat message actions → quota visibility → accessibility labels → logout confirmation → New-chat entry point → auth-form gaps.
...
- [x] **User-facing on/off switch** (Settings → Notifications). The stored preference gates auto-registration, which also closes a bug in the first cut: `hydrate()` re-registers on every launch, so without this gate turning notifications off would have silently switched itself back on at next app start. No separate "muted" flag server-side — **absence of a token is the off switch**. Deliberate design call: the preference records *intent* and is left untouched when the environment blocks push (Expo Go, revoked OS permission), so installing a dev build or re-allowing the permission just starts working rather than requiring the user to come back and re-toggle. A permanent OS denial is detected and offers to open system settings instead of silently flipping the switch back.
