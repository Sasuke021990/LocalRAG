...
import { KeyRound, Webhook, ChevronDown } from 'lucide-vue-next'
...
<template>
  <div class="flex flex-col gap-8 max-w-2xl">
    ...
    <Card>
      <button type="button" class="flex items-center gap-3 w-full text-left cursor-pointer" @click="passwordOpen = !passwordOpen">
        ...
        <ChevronDown class="w-4 h-4 text-ink-muted transition-transform shrink-0" :class="passwordOpen ? 'rotate-180' : ''" />
      </button>
      <form v-if="passwordOpen" class="flex flex-col gap-4 mt-5" @submit.prevent="changePassword">
